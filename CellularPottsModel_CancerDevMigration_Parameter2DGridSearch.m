...
% 
% par1_list = 10.^linspace(-1.5,3,6);
% par2_list = 10.^linspace(-1.5,3,6);

% local vs global remodeling
% par1_list = [5, 45];       % global remodeling
% par2_list = [0]; % local remodeling

% % local vs jcc
% par1_list = [5, 10, 40, 45];       % global remodeling
...
% par2_list = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1028];    % gamma

% % old %
% par1_list = [1];        % spheroid
% par2_list = [5];
% par1_list = [0.2];      % network
% par2_list = [50];

% recent %
% par1_list = [80, 165, 330, 660, 1320];        % jce
% par2_list = [33, 330, 3300];                  % rho_jma_k

% par1_list = [1.1];      % spheroid, q.rho
% par2_list = [1.2];        % spheroid, q.KD1
% par1_list = [0.45];      % network, q.rho
% par2_list = [41];        % network, q.KD1
% par1_list = [1];      % hybrid
% par2_list = [15];

% % global remodeling due to growth %
% par1_list = [0.5];        % spheroid
% par2_list = [0];


% fiber denisty and cell-cell adhesion %
% original jcc = 300
...
if savemovie
    if parsave
        v = VideoWriter(sprintf('mov_%s%0.2e_%s%0.2e_%s',par1name,par1,par2name,par2,num2str(repeatN_idx)),'MPEG-4');
    else
...
if twocell_tf > tN
    ...
    close(FIG)
    save(sprintf('sim_%s%0.2e_%s%0.2e_%s_data.mat',par1name,par1,par2name,par2,num2str(repeatN_idx)))
    
    run_counter = run_counter + 1;
    fprintf('%s:%0.2e, %s:%0.2e, repeat:%s [%d/%d] %s \n',par1name,par1,par2name,par2,num2str(repeatN_idx),run_counter,length(par1_list)*length(par2_list)*repeatN,exitnote);
...
run_counter = run_counter + 1;
fprintf('%s:%0.2e, %s:%0.2e, repeat:%s [%d/%d]\n',par1name,par1,par2name,par2,num2str(repeatN_idx),run_counter,length(par1_list)*length(par2_list)*repeatN);
        end
    end
end
